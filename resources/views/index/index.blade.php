...
@section('content')
     ...
     <div id="sliderA" class="slider">
      @foreach($rew as $k=>$v)
      <img src="http://1908uploads.com/{{$v->g_img}}" />
      @endforeach
     </div><!--sliderA/-->
     <ul class="pronav">
        @foreach($res as $k=>$v)
        <li><a href="{{url('/porlist/'.$v->t_id)}}">{{$v->t_name}}</a></li>
        @endforeach
      <div class="clearfix"></div>
     </ul><!--pronav/-->
     <div class="index-pro1">
      ...
      <div class="index-pro1-list">
       <dl>
        <dt><a href="{{url('/porinfo/'.$v->g_id)}}"><img src="http://1908uploads.com/{{$v->g_img}}" width="650"/></a></dt>
        <dd class="ip-text"><a href="{{url('/porinfo/'.$v->g_id)}}">{{$v->g_name}}</a><span>已售：488</span></dd>
        <dd class="ip-price"><strong>¥{{$v->g_pirate}}</strong> <span>¥599</span></dd>
       </dl>
      </div>
      @endforeach
      <div class="clearfix"></div>
